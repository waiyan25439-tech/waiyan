...
async def fetch_free_proxies():
    """Public Proxy API များမှ အော်တို Proxy ယူရန်"""
    global active_proxies
    urls = [
        "https://raw.githubusercontent.com/clashstash/proxy-scraper/main/proxies/http.txt",
            ...
            try:
                async with session.get(url, timeout=10) as resp:
                    if resp.status == 200:
                        ...
                        found = re.findall(r'\b\d{1,3}(?:\.\d{1,3}){3}:\d{2,5}\b', text)
                        for p in found:
                            temp_proxies.add(f"http://{p}")
            except Exception:
                continue
                
    if temp_proxies:
        active_proxies = list(temp_proxies)
        random.shuffle(active_proxies)
        print(f"[*] Successfully loaded {len(active_proxies)} proxies.")
    else:
        # Fallback if APIs fail (Empty means direct connection or fallback)
        active_proxies = []
        print("[!] Warning: Could not fetch proxies, running without proxy or check connection.")

async def get_next_proxy():
    """Proxy များကို လှည့်ပတ်သုံးရန် (Round-robin)"""
    global current_proxy_index, active_proxies
    if not active_proxies:
        return None
    async with proxy_lock:
        if current_proxy_index >= len(active_proxies):
            current_proxy_index = 0
        proxy = active_proxies[current_proxy_index]
...
async def get_balance_info(session_id, proxy=None):
    ...
    async with aiohttp.ClientSession() as temp_session:
        for url in endpoints:
            try:
                async with temp_session.get(url, headers=headers, proxy=proxy, timeout=8) as resp:
                    if resp.status != 200:
                        continue
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        "🔥 **RUIJIE EXTREME SCANNER BOT (WITH AUTO PROXY)** 🔥\n\n"
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔄 Proxy များကို Refresh လုပ်ရန်", callback_data="refresh_proxies")],
    ...
    current_proxy_display = "None"
    if proxy_count > 0:
        current_proxy_display = f"{active_proxies[current_proxy_index - 1 % proxy_count][:25]}..." if proxy_count > 0 else "Active"

    msg = (
        f"⚙️ **Dashboard (Auto Proxy System)**\n\n"
        f"🔗 **URL:** `{state['url'][:30]}...`\n"
        f"📊 **Selected Mode:** `{state['mode']}`\n"
        f"🛡️ **Loaded Proxies:** `{proxy_count}` proxies\n"
        f"⚡ **Status:** {status}\n"
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "select_mode":
        ...
        await query.edit_message_text("🔄 Proxy အသစ်များ ဆွဲထုတ်နေပါပြီ ခဏစောင့်ပါ...")
        ...
        await query.edit_message_text("🚀 **Scan စတင်နေပါပြီ (Proxy Rotation Enabled)...**")
...
async def run_scan_task(user_id, context, chat_id, message_id):
    ...
    try:
        while state['running']:
            ...
            # ၅ စက္ကန့်တစ်ခါ Telegram Progress Update လုပ်ပေးခြင်း (UI တွင် Proxy အရေအတွက်နှင့် အခြေအနေပြရန်)
            if time.time() - last_update_time > 5:
                ...
                status_msg = (
                    f"⚡ **Scanning In Progress (Auto Proxy)...**\n\n"
                    f"📊 **Mode:** `{mode}`\n"
                    f"🛡️ **Active Proxies:** `{proxy_count}`\n"
                ...
                keyboard = [
                    [InlineKeyboardButton("🔄 Proxy Refresh", callback_data="refresh_proxies")],
                    [InlineKeyboardButton("⏹️ ရပ်တန့်ရန်", callback_data="stop_scan")]
                ]
                try:
...
# ── MAIN BOT RUNNER ────────────────────────────────________________──────
...
def main():
    ...
    print("🤖 Telegram Bot Starting (Auto-Proxy & Admin Security Active)...")
    app.run_polling()
