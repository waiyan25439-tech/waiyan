#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# KHEANG - Ruijie Extreme Telegram Bot Scanner (Auto-Proxy & Speed Integrated)[span_1](start_span)[span_1](end_span)
...
# ── CONFIGURATION ──────────────────────────────────────────────────────────
BOT_TOKEN = "8648844261:AAGohP2qqSsDzcVTM42V9adhb08hLvsCZbM"  # <--- သင့် Bot Token
ADMIN_ID = 8635066797                                      # <--- Single Admin ID (သင့်အကောင့်)

BATCH_SIZE = 2000        # Telegram Bot အတွက် သင့်တော်သော Batch Size
MAX_CONCURRENT = 300     # ⚡ Speed & Proxies: Async/Await ဖြင့် တစ်ပြိုင်နက်တည်း Request အများကြီး (Concurrent Requests) ပစ်ရန်
...
# Proxy Pool အတွက် Global စာရင်း
ACTIVE_PROXIES = []
...
# ── AUTO-PROXY FETCHER & ROTATOR ─────────────────────────────────────────
async def fetch_auto_proxies():
    """
    🌐 IP Block မခံရခြင်း: Public proxy source များမှ Proxy များကို အော်တိုဖမ်းယူပေးပြီး
    Ruijie server ဘက်မှ IP ကို ဘလော့ချခြင်းမရှိစေဘဲ ဆက်တိုက် ရှာဖွေနိုင်စွမ်းကို မြှင့်တင်ပေးသည်။
    """
    proxy_urls = [
...
async def update_proxy_pool():
    ...
    fetched = await fetch_auto_proxies()
    if fetched:
        ACTIVE_PROXIES = fetched
...
async def perform_check(session_url, code):
    """
    ⚡ Speed & Proxies: Async/Await (aiohttp) ကိုသုံးပြီး တစ်ပြိုင်နက်တည်း Request အများကြီး (Concurrent Requests) ပစ်နိုင်အောင် 
    တည်ဆောက်ထားပြီး Proxy များကို အော်တိုဖမ်းဆွဲကာ လှည့်သုံးပေးခြင်းဖြင့် IP Block မခံရဘဲ အမြန်ဆုံးနှင့် အမှန်ကန်ဆုံး ရှာဖွေပေးသည်။
    """
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Bot စတင်ချိန်တွင် Proxy Pool ကို အော်တို update လုပ်ရန်
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        f"⚙️ **Dashboard (Auto-Proxy & Speed Enabled)**\n\n"
        f"🔗 **URL:** `{state['url'][:40] if state['url'] else 'None'}...`\n"
        f"📊 **Selected Mode:** `{state['mode']}`\n"
        f"⚡ **Status:** {status}\n"
        f"🛡️ **Active Proxies:** {len(ACTIVE_PROXIES)}\n"
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "select_mode":
        ...
        # Scan မစတင်မီ Proxies များကို နောက်ခံဖြင့် အသစ်ထပ်ဆွဲရန်
        asyncio.create_task(update_proxy_pool())
        ...
        await query.edit_message_text("🚀 **Scan စတင်နေပါပြီ (Auto-Proxies & Speed Active)...**")
    ...
    try:
        while state['running']:
            ...
            if time.time() - last_update_time > 5:
                ...
                status_msg = (
                    f"⚡ **Scanning In Progress (Speed & Proxies Active)...**\n\n"
                    f"📊 **Mode:** `{mode}`\n"
                    f"📦 **Checked:** {state['checked']:,}\n"
                    f"⚡ **Speed:** {speed:,.0f} /min\n"
                    f"🛡️ **Proxies:** {len(ACTIVE_PROXIES)} loaded\n"
...
async def post_init(application: Application):
    ...
    # Bot စတင်ချိန်တွင် Proxies များကို ကြိုတင်ဖမ်းဆွဲထားရန်
...
def main():
    ...
    print("🤖 Telegram Bot Starting with Auto-Proxy & Speed Optimization...")
    app.run_polling()
