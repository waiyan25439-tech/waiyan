...
async def check_channel_membership(user_id: int, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        ...
        # Check if user status is valid (member, administrator, or creator/owner)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # 1. Check Channel Subscription First
        ...
        if not is_member:
            ...
            await update.message.reply_text(
                f"❌ ကျေးဇူးပြု၍ Bot ကို အမသုံးစွဲရန် ပထမဦးစွာ ကျွန်ုပ်တို့၏ချန်နယ်ကို **Join** လုပ်ထားပေးပါရန်။\n\n"
        ...
        if not can_use:
            await update.message.reply_text(
                f"❌ သင့်ရဲ့ နေ့စဉ် အသုံးပြုခွင့် ၁၅ မိနစ် ကုန်ဆုံးသွားပါပြီ[span_0](start_span)[span_0](end_span)။\n"
                f"ကျေးဇူးပြု၍ Key ဝယ်ယူပြီး အသုံးမပြုလိုပါက သို့မဟုတ် ထပ်မံအသုံးပြုလိုပါက {ADMIN_CONTACT_USERNAME} ထံသို့ ဆက်သွယ်ဝယ်ယူပါ[span_1](start_span)[span_1](end_span)။\n\n"
        ...
        if is_admin(user_id):
            keyboard = [
                [InlineKeyboardButton("🛠️ Admin Panel သို့ဝင်ရန်", callback_data="admin_panel")][span_2](start_span)[span_2](end_span),
...
async def redeem_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🎁 သင့်အား **{days} ရက်စာ** အသုံးပြုခွင့် ပေးအပ်လိုက်ပါပြီ[span_3](start_span)[span_3](end_span)။\n"
        ...
        if is_admin(user_id) and context.user_data.get('waiting_broadcast'):
            ...
            for uid in all_users:
                try:
                    ...
                    pass
            await update.message.reply_text(f"✅ User {count} ဦးထံသို့ စာပို့ပြီးပါပြီ[span_4](start_span)[span_4](end_span)။")
            return

        can_use, _ = check_user_limit(user_id)
        if not can_use:
            await update.message.reply_text(f"❌ အသုံးပြုခွင့် ကုန်ဆုံးသွားပါပြီ[span_5](start_span)[span_5](end_span)။ {ADMIN_CONTACT_USERNAME} ထံတွင် Key ဝယ်ယူနိုင်ပါသည်[span_6](start_span)[span_6](end_span)။")
            return
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if is_admin(user_id):
            keyboard.append([InlineKeyboardButton("🛠️ Admin Panel", callback_data="admin_panel")])[span_7](start_span)[span_7](end_span)
    ...
    try:
        ...
        if data == "admin_panel":
            if not is_admin(user_id):
                ...
                return
            keyboard = [
                [InlineKeyboardButton("👥 Bot သုံးနေသူ စုစုပေါင်း ကြည့်ရန်", callback_data="admin_stats")][span_8](start_span)[span_8](end_span),
                [InlineKeyboardButton("📡 Scan ဖတ်နေသူများ ကြည့်ရန်", callback_data="admin_active_scans")][span_9](start_span)[span_9](end_span),
                ...
                [InlineKeyboardButton("✉️ User အားလုံးကို စာပို့ရန် (Broadcast)", callback_data="admin_broadcast")][span_10](start_span)[span_10](end_span),
        ...
        if data in ["gen_key_15", "gen_key_20", "gen_key_30"]:
            ...
            text = f"🔑 **{days} ရက်စာ Key အသစ်ထုတ်ပေးပြီးပါပြီ[span_11](start_span)[span_11](end_span):**\n\n`{key_code}`\n\n(ဤ Key ကို ဝယ်ယူသူထံ ပေးပို့နိုင်ပါသည်[span_12](start_span)[span_12](end_span))"
            keyboard = [
        ...
        if data == "admin_stats":
            ...
            text = f"📊 **Bot Statistics**\n\n👤 စုစုပေါင်း Users: `{len(all_users)}` ဦး[span_13](start_span)[span_13](end_span)\n⚡ Active Scans: `{active_scans_count}` ဦး[span_14](start_span)[span_14](end_span)"
            keyboard = [[InlineKeyboardButton("⬅️ Back", callback_data="admin_panel")]]
            await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
            return

        if data == "admin_active_scans":
            text = "📡 **လက်ရှိ Scan ဖတ်နေသူများ[span_15](start_span)[span_15](end_span):**\n\n"
            count = 0
        ...
        if data == "admin_broadcast":
            ...
            text = "✉️ User အားလုံးဆီ ပို့လိုသော **စာသား** ကို ပို့ပါ။[span_16](start_span)[span_16](end_span)"
            keyboard = [[InlineKeyboardButton("⬅️ Back", callback_data="admin_panel")]]
            await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
            return
        ...
        while state['running']:
            ...
            if not can_use:
                ...
                await context.bot.send_message(
                    chat_id=chat_id, 
                    text=f"⏰ အသုံးပြုခွင့် သက်တမ်း ကုန်ဆုံးသွားပါပြီ[span_17](start_span)[span_17](end_span)။ {ADMIN_CONTACT_USERNAME} ထံတွင် Key ထပ်မံဝယ်ယူပါ[span_18](start_span)[span_18](end_span)။"
                )
                break
