...
# ── CONFIGURATION ──────────────────────────────────────────────────────────
BOT_TOKEN = "8648844261:AAGb737MMCYlLYUPG2uzHNC7TsiFi1SX2sY"  # <--- သင့် Bot Token
...
DEFAULT_FREE_LIMIT = 15 * 60  # Free user များအတွက် တစ်နေ့လျှင် ၁၅ မိနစ် (စက္ကန့်ဖြင့်)[span_2](start_span)[span_2](end_span)
...
user_scans = {}          # User တစ်ယောက်ချင်းစီ၏ Scan အခြေအနေများ[span_3](start_span)[span_3](end_span)
user_usage = {}          # User တစ်ဦးချင်းစီ၏ နေ့စဉ် အသုံးပြုခဲ့သည့် အချိန်များ[span_4](start_span)[span_4](end_span)
all_users = set()        # Bot ကို /start လုပ်ထားသူ User အားလုံး၏ ID များ[span_5](start_span)[span_5](end_span)
generated_keys = {}      # Admin မှ ထုတ်ပေးထားသော Key များ သိမ်းရန် (Format: {key_code: days_duration})[span_6](start_span)[span_6](end_span)
user_subscriptions = {}  # Key ထည့်ထားသော User များ၏ သက်တမ်းကုန်မည့် အချိန် (Format: {user_id: expire_timestamp})[span_7](start_span)[span_7](end_span)
...
def check_user_limit(user_id: int) -> tuple[bool, str]:
    """User တစ်ဦးချင်းစီ၏ အသုံးပြုခွင့်ကို စစ်ဆေးသည် (VIP Key ရှိမရှိ သို့မဟုတ် Free ၁၅ မိနစ် ကုန်ဆုံးခြင်း)"""
    if is_admin(user_id):
        return True, "VIP Admin"

    current_time = time.time()
    
    # 1. VIP Subscription ရှိမရှိ စစ်ဆေးရန်
    if user_id in user_subscriptions:
        if current_time < user_subscriptions[user_id]:
            ...
            # ရက်ကုန်သွားပါက ဖယ်ရှားမည်
            del user_subscriptions[user_id]

    # 2. Free User နေ့စဉ် ၁၅ မိနစ် ကန့်သတ်ချက် စစ်ဆေးရန်
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        all_users.add(user_id)  #[span_8](start_span)[span_8](end_span)

        can_use, status_text = check_user_limit(user_id)
        if not can_use:
            await update.message.reply_text(
                f"❌ သင့်ရဲ့ နေ့စဉ် အသုံးပြုခွင့် ၁၅ မိနစ် ကုန်ဆုံးသွားပါပြီ[span_9](start_span)[span_9](end_span)။\n"
                f"ကျေးဇူးပြု၍ Key ဝယ်ယူပြီး အသုံးမပြုလိုပါက သို့မဟုတ် ထပ်မံအသုံးပြုလိုပါက {ADMIN_CONTACT_USERNAME} ထံသို့ ဆက်သွယ်ဝယ်ယူပါ[span_10](start_span)[span_10](end_span)။\n\n"
        ...
        if is_admin(user_id):
            keyboard = [
                [InlineKeyboardButton("🛠️ Admin Panel သို့ဝင်ရန်", callback_data="admin_panel")][span_11](start_span)[span_11](end_span),
...
async def redeem_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ဝယ်ယူသူများ Key ထည့်သွင်းအသုံးပြုရန် Command (/redeem KEY)"""
    user_id = update.effective_user.id
    args = context.args
    if not args:
    ...
    # Key ကို အသုံးချပြီး ရက်ပေါင်းအတိုင်း သက်တမ်းတိုးပေးခြင်း
    days = key_data['days']
    key_data['used'] = True
    
    current_time = time.time()
    # လက်ရှိ VIP ရှိပြီးသားဆိုရင် ပေါင်းထည့်မည်၊ မရှိရင် လက်ရှိအချိန်မှစမည်
    ...
    await update.message.reply_text(
        ...
        f"🎁 သင့်အား **{days} ရက်စာ** အသုံးပြုခွင့် ပေးအပ်လိုက်ပါပြီ[span_12](start_span)[span_12](end_span)။\n"
        ...
        if is_admin(user_id) and context.user_data.get('waiting_broadcast'):
            ...
            for uid in all_users:
                try:
                    ...
                    pass
            await update.message.reply_text(f"✅ User {count} ဦးထံသို့ စာပို့ပြီးပါပြီ[span_13](start_span)[span_13](end_span)။")
            return

        can_use, _ = check_user_limit(user_id)
        if not can_use:
            await update.message.reply_text(f"❌ အသုံးပြုခွင့် ကုန်ဆုံးသွားပါပြီ[span_14](start_span)[span_14](end_span)။ {ADMIN_CONTACT_USERNAME} ထံတွင် Key ဝယ်ယူနိုင်ပါသည်[span_15](start_span)[span_15](end_span)။")
            return
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if is_admin(user_id):
            keyboard.append([InlineKeyboardButton("🛠️ Admin Panel", callback_data="admin_panel")])[span_16](start_span)[span_16](end_span)
    ...
    try:
        ...
        if data == "admin_panel":
            if not is_admin(user_id):
                ...
                return
            keyboard = [
                [InlineKeyboardButton("👥 Bot သုံးနေသူ စုစုပေါင်း ကြည့်ရန်", callback_data="admin_stats")][span_17](start_span)[span_17](end_span),
                [InlineKeyboardButton("📡 Scan ဖတ်နေသူများ ကြည့်ရန်", callback_data="admin_active_scans")][span_18](start_span)[span_18](end_span),
                ...
                [InlineKeyboardButton("✉️ User အားလုံးကို စာပို့ရန် (Broadcast)", callback_data="admin_broadcast")][span_19](start_span)[span_19](end_span),
        ...
        if data in ["gen_key_15", "gen_key_20", "gen_key_30"]:
            ...
            text = f"🔑 **{days} ရက်စာ Key အသစ်ထုတ်ပေးပြီးပါပြီ[span_20](start_span)[span_20](end_span):**\n\n`{key_code}`\n\n(ဤ Key ကို ဝယ်ယူသူထံ ပေးပို့နိုင်ပါသည်[span_21](start_span)[span_21](end_span))"
            keyboard = [
        ...
        if data == "admin_stats":
            ...
            text = f"📊 **Bot Statistics**\n\n👤 စုစုပေါင်း Users: `{len(all_users)}` ဦး[span_22](start_span)[span_22](end_span)\n⚡ Active Scans: `{active_scans_count}` ဦး[span_23](start_span)[span_23](end_span)"
            keyboard = [[InlineKeyboardButton("⬅️ Back", callback_data="admin_panel")]]
            await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
            return

        if data == "admin_active_scans":
            text = "📡 **လက်ရှိ Scan ဖတ်နေသူများ[span_24](start_span)[span_24](end_span):**\n\n"
            count = 0
        ...
        if data == "admin_broadcast":
            ...
            text = "✉️ User အားလုံးဆီ ပို့လိုသော **စာသား** ကို ပို့ပါ။[span_25](start_span)[span_25](end_span)"
            keyboard = [[InlineKeyboardButton("⬅️ Back", callback_data="admin_panel")]]
            await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
            return
        ...
        while state['running']:
            ...
            if not can_use:
                ...
                await context.bot.send_message(
                    chat_id=chat_id, 
                    text=f"⏰ အသုံးပြုခွင့် သက်တမ်း ကုန်ဆုံးသွားပါပြီ[span_26](start_span)[span_26](end_span)။ {ADMIN_CONTACT_USERNAME} ထံတွင် Key ထပ်မံဝယ်ယူပါ[span_27](start_span)[span_27](end_span)။"
                )
                break
            ...
            # အကယ်၍ Free user ဖြစ်ပါက နေ့စဉ် အသုံးပြုချိန်ကို တွက်ချက် မှတ်သားမည်
...
def main():
    ...
    app.add_handler(CommandHandler("redeem", redeem_command))  # Key ထည့်ရန် command အသစ်
    app.add_handler(CallbackQueryHandler(callback_handler))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))
    
    app.add_error_handler(error_handler)
    
    print("🤖 Telegram Bot Running with 15/20/30 Days Key Redeem System...")
